...
1. **Read client context:**
   ...
   - `company-overview.md` for business context
   - All knowledge files in `marketing/knowledge/`

2. **Review recent performance:**
   - Read `marketing/knowledge/whats-working.md` for current patterns
   - Check `marketing/knowledge/goals-and-benchmarks.md` for 90-day goals and current priorities
   - If `marketing/tracking/performance.csv` has recent data, analyze last 30 days for trends

3. **Check for an existing monthly plan:**
   - Look in `marketing/outputs/monthly-briefs/` for the current month
...
4. **Generate the plan using the Monthly Plan Template** (see `output-formats.md` for the full template):
   ...
   - Align key messages with messaging pillars from `marketing/knowledge/voice-guidelines.md`

5. **Save the plan:**
   - Markdown: `marketing/outputs/monthly-briefs/YYYY-MM-monthly-plan.md`
...
- Balance what's proven (from `marketing/knowledge/whats-working.md`) with what needs testing. Roughly 70% proven approaches, 30% experiments.
...
3. **Review what's working:**
   - Read `marketing/knowledge/whats-working.md` for hook patterns, best times, top content types
   - Check `marketing/knowledge/goals-and-benchmarks.md` for current campaign focus

4. **If performance data exists**, review last week's performance:
   - Read recent entries in `marketing/tracking/performance.csv`
...
5. **Generate the plan using the Weekly Plan Template** (see `output-formats.md` for the full template):
   ...
   - For each piece: platform, format, topic, hook, key message, CTA, and a "why this works" note referencing `marketing/knowledge/whats-working.md`
...
6. **Save the plan:**
   - Markdown: `marketing/outputs/weekly-briefs/YYYY-MM-DD-weekly-plan.md`
   - If client has alternate output formats configured, follow those instructions from client `.claude/CLAUDE.md`
...
- Hooks are the most important element. Reference proven hook patterns from `marketing/knowledge/whats-working.md`.
