...
**Captions:** NEVER use a single wide table. Break into individual blocks per post:
```
### Post 1 — Title (Day Date)
[Caption text as readable paragraphs]
`#Hashtags`
```

**Scripts:** Use consistent simple tables (Section | Time | Visual | Audio). Never use ASCII box-drawing characters or inconsistent indentation.
...
**Structure order:** Overview -> Posting Schedule -> Captions (per-post blocks) -> Content Details (per-post) -> Content Bank -> Scripts -> Shoot Recap -> Cross-Platform Notes -> Checklists -> Project Reference
