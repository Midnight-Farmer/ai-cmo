...
Detailed configuration for external services. **All integrations are optional** and configured per-client. The AI-CMO system works fully with local markdown and CSV files.
...
2. **Social Set ID:**
   ...
   - Create `marketing/knowledge/typefully-config.md` for each client that uses Typefully:
     ```markdown
...
- Write JSON payloads to `/tmp/typefully-draft.json` to avoid shell escaping issues
...
For detailed API procedures, see `references/typefully.md`.
